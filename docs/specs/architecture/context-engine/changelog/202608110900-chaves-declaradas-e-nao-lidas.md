...
6 chaves desta spec foram removidas:

- `DCODE_BUDGET_BANDS`
- `DCODE_COMPACT_AT`
- `DCODE_COMPACT_ENABLED`
- `DCODE_COMPACT_KEEP_TURNS`
- `DCODE_TOKEN_ESTIMATE_MARGIN`
- `DCODE_TOKEN_ESTIMATE_RATIO`
...
**Remover a chave não remove o comportamento.** `CompactAt` continua em `0.80` e `CharsPerToken` em `3.5`, fixos em `DefaultConfig()`. O que sai é a promessa de que dá para mudá-lo por configuração, promessa que nunca foi verdadeira.
